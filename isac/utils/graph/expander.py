...
class BronKerboschExpander(object):
    ...
    def _build_graph(self, tags):
        ...
        for tag_index in xrange(len(tags)):
            for entity_index in xrange(len(tags[tag_index].get('entities'))):
                ...
                tokens = self.tokenizer.tokenize(tags[tag_index].get(
        ...
        for clique in get_cliques(list(entities), graph):
            ...
            for entity_name in clique:
                ...
                tag = {
                    ...
                    'confidence': entities.get(
                        entity_name)[1] * old_tag.get('confidence', 1.0),
    ...
    def expand(self, tags, scoring_func=None):
        ...
        for i in xrange(len(tags)):
            ...
            if len(overlapping_spans) > 0 and end_token_index() \
                ...
                if scoring_func:
                    cliques = sorted(
                        cliques, key=lambda e: -1 * scoring_func(e))
